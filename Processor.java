...
public class Processor{
	...
	public void determineAction(String[] words){
		...
		if (currentLoc.getY() == 4 && currentLoc.getX() == 1){
			endState(words);
			return;
		}
		if (!commandList.containsKey(words[0])){
			System.out.println("Theres no point in doing that..");
			return;
		}
		if (commandList.get(words[0]).equals("go")){
			String secondWord = secondWordList.get(words[1]);
			action.setWord2(secondWord);
			if(secondWord == null){
				System.out.println("Go where?");
			}else{
				if(!currentLoc.checkPos(secondWord)){
					System.out.println("You bump into a wall, good job detective");
				}else{
					currentLoc.setPos(secondWord);
					action.setLocation(currentLoc);
					action.displayState(loc, showHint); // Shows the new location you are in
					currentInv = action.displayItems(loc);
				}
			}

		}else if (words[0].equals("examine")){ // currentInv is the ground inventory
			if (words[1].equals("body") && currentLoc.getX() == 0 && currentLoc.getY() == 2) {
				System.out.println(newItem[7].getDescription());
				return;
			}
			currentItem = action.getItem(currentInv, words[1]);
			if (currentItem != null){
				System.out.println(currentItem.getDescription());
			}else if (myInventory.containsKey(words[1])){
				System.out.println(myInventory.get(words[1]).getDescription());
			}else {
				System.out.println("No item to examine");
			}			
		}else if (words[0].equals("take")){
			if (words[1].equals("body") && currentLoc.getX() == 0 && currentLoc.getY() == 2) {
				System.out.println("Seems too heavy to put in the bag dontcha think..");
				return;
			}
			currentItem = action.getItem(currentInv, words[1]);
			if (currentItem != null){
				myInventory.put(words[1], currentItem);
				System.out.println("You placed " + words[1] + " in your inventory for evidence");
				action.removeItem(currentInv, words[1]);
			}else{
				System.out.println("No item to take");
			}
		}else if (words[0].equals("drop")){
			if(myInventory.containsKey(words[1])){
				myInventory.remove(words[1]);
				System.out.println("You have dropped " + words[1]);
				System.out.println("FYI you'll probably never see it again");
			}else{
				System.out.println("You don't have that object");
			}
		}else if (words[0].equals("look")){
			if(!currentLoc.checkPos(words[1])){
				System.out.println("You see a wall, looks fancy but needs to be repainted");
			}else{
				action.displayLook(loc, currentLoc, words[1]); // Shows the new location you are in
			}
		}else if (words[0].equals("use")){
			if(!myInventory.containsKey(words[1])){
				System.out.println("Uh you don't have that item");
			}else{
				Item thatItem = myInventory.get(words[1]);
				//System.out.println(thatItem.getAction());
				action.useItem(thatItem); // Shows the new location you are in
			}
		}else if (words[0].equals("show")) {
			if (words[1].equals("commands")){
				System.out.println("Commands: What they do");
				System.out.println("go: How you would move in this game..Directions (north, south, west, east or up down left right)");
				System.out.println("use: Items have various useage in certain locations");
				System.out.println("take: pick up items in your inventory");
				System.out.println("drop: drops your item in your inventory(officially disappears forever)");
				System.out.println("examine: examine items to see more information ");
				System.out.println("look: looks in a certain direction to see the rooom before you enter ");
				System.out.println("show hint: maps out your location to make it easier to move around");
				System.out.println("hide hint: makes the game harder, you hide the hint");
			}else if (words[1].equals("hint")){
				showHint = true;
			}else{
				System.out.println("Cant show that..but if you need help try: show commands");
			}
		}else if (words[0].equals("hide") && words[1].equals("hint")){
			showHint = false;
		}else{	
			System.out.println("Theres no point in doing that..");
		}
